...
class Sensor:
    ...
    def in_fov(self, x):
        ...
        #take vehicle coordinates as a 4x1 column matrix of ones
        veh_coord = np.ones((4, 1))
        #retrieve them from the x
        #print("printing x first 4 coord")
        #print(x[0:3])
        veh_coord[0:3] = x[0:3] 
        #we can multiply this with 1x4 matrix of the class variable to get 1x1 value of the sensor coordinates
        #this will trasform vehicle coordinates to lidar coordinates
        
        sensor_pos = self.veh_to_sens * veh_coord

        #storing first 3 elements as coordinates

		#angle can be calculated using math Lib function
        if sensor_pos[0] > 0:
            angle = np.arctan(sensor_pos[1] / sensor_pos[0])
		#conditions for the angle between the field of view
    ...
    def get_hx(self, x):    
        ...
        if self.name == 'lidar':
            ...
            pos_veh = np.ones((4, 1)) # homogeneous coordinates as above in lidar
            pos_veh[0:3] = x[0:3]  #same as above in lidar
            hx = np.ones((2,1))
            sensor_pos = self.veh_to_sens * pos_veh #vehicle to lidar coordinates transformation
            #x, y and z can be separately taken out as follow
            x, y, z = sensor_pos[0:3]
            # - project from camera to image coordinates
            if x <= 0:
                hx = np.array([-100, -100])
            else:   
                temp_1 = y / x
                hx[0, 0] = (self.c_i - self.f_i) * temp_1
                
                temp_2 = z / x
                hx[1, 0] = self.c_j - self.f_j * temp_2
    ...
    #     removing this to remove restriction to lidar to include camera as well    if self.name == 'lidar':
...
class Measurement:
    ...
    def __init__(self, num_frame, z, sensor):
        ...
        if sensor.name == 'lidar':
            ...
            #creating a z coord vector
            self.z = np.zeros((sensor.dim_meas,1)) # measurement vector
			#filling up the values from the passed argument
            self.z[0][0] = z[0]
            self.z[1][0] = z[1]
            self.sensor = sensor # measurement generating sensor, e.g. lidar, camera etc
            sigma_cam_i = params.sigma_cam_i
            sigma_cam_j = params.sigma_cam_j
            self.R = np.matrix([[sigma_cam_i**2, 0], # matrix for covariance
                                [0, sigma_cam_j**2]])
